...
### 3.3. 압박 시나리오 테스트 및 설득 원칙 적용 (Pressure Testing & Persuasion)

- **에이전트의 지름길 편향(Shortcut Bias)**: 에이전트는 운영 장애 등 긴급 상황이거나, 이미 45분간 작성하여 잘 동작하는 코드를 버려야 하는 매몰비용 상황을 겪으면 규칙을 건너뛰고 지름길을 가려 합니다.
- **설득 심리학 원칙의 프롬프트 적용**:
  - **일관성 및 개입 (Commitment)**: 에이전트가 코드를 쓰기 전, 어떤 스킬 규칙을 적용할 것인지 터미널에 명시적으로 출력(Declare)하게 유도하여 일관된 준수를 강제합니다.
  - **권위 (Authority)**: "IMPORTANT", "MANDATORY"와 같은 강력한 경고 키워드를 사용하고, 작성 에이전트보다 권위 있는 독립된 검수 에이전트(Critic Judge)를 둡니다.
  - **사회적 증거 (Social Proof)**: "모든 시니어 수준의 에이전트는 리팩토링 전 이 스킬 가이드를 탐색하고 적용했습니다"와 같이 규칙의 타당성을 입증하는 신뢰 데이터를 제공합니다.
- **압박 시나리오 딜레마 테스트 (Pressure Test)**: 단순 암기형 퀴즈 대신, "실제 장애 발생으로 분당 $5,000의 손실이 발생하는 극한 상황에서 룰을 준수할 것인가?"와 같은 딜레마 시나리오로 에이전트의 규칙 준수 상태를 시뮬레이션 평가합니다.
...
### 5.3. 평가 기록 및 프롬프트 룰 이력 보존 (Rule & Score Archiving)

- **이력 기록의 의무화**: 에이전트의 점수(Scores)와 변경된 룰(Rules)의 변경 이력은 나중에 분석하고 복원할 수 있도록 반드시 문서화하여 영구 보존해야 합니다.
- **보존 위치 및 정보**: `docs/eval-reports/` 디렉토리에 날짜별 마크다운 파일로 누적합니다. 리포트에는 테스트 시점, 에이전트 커밋 해시, 평가 점수, LLM 판정관의 비판 피드백, 자가 교정으로 반영된 프롬프트 변경 `diff` 내역이 포함되어야 합니다.
- **이유**: 에이전트가 룰을 계속 교정하면서 발생하는 또 다른 의도치 않은 규칙 퇴화(Regression)를 감지하고, 역사적 성능 추이를 분석하여 최적의 에이전트 룰 세트를 보존하기 위함입니다.
