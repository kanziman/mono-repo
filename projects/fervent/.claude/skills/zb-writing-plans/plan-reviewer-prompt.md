...
## 검토 대상
- 계획서 파일 경로: [PLAN_FILE_PATH] (반드시 docs/exec-plans/active/ 하위에 위치해야 함)
- 체크리스트 JSON 파일 경로: [CHECKLIST_JSON_PATH] (반드시 docs/exec-plans/active/ 하위에 위치해야 함)

## 검증 체크리스트
1. **위치 및 매핑 검증**: 계획서와 체크리스트 JSON이 모두 `docs/exec-plans/active/` 폴더에 제대로 생성되었는가?
