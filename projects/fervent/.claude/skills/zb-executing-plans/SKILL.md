...
2. `[ ]` **계획 로드 및 검토**:
   - `docs/exec-plans/active/[plan-name]-plan.md` 및 `[plan-name]-checklist.json`을 읽고 분석합니다.
...
4. `[ ]` **생명주기(Lifecycle) 마감**:
   ...
   - 활성(`active`) 상태인 계획서와 체크리스트 파일을 완료(`completed`) 디렉토리로 이동시킵니다.
     - `docs/exec-plans/active/[plan-name]-plan.md` ➡️ `docs/exec-plans/completed/[plan-name]-plan.md`
     - `docs/exec-plans/active/[plan-name]-checklist.json` ➡️ `docs/exec-plans/completed/[plan-name]-checklist.json`
5. `[ ]` **통합 및 정리**:
   - 최종 작업물 확인 후, 통합(Git PR/머지) 단계는 프로젝트 브랜치 정리 규칙(`zb-finishing-a-development-branch` 등)을 따릅니다.
