...
## 집중 검증 항목
1. **의존성 및 아키텍처 규칙 위반**: UI가 DB/영구 저장소에 직접 접근하는지, 의존성 방향이 안쪽으로 흐르는지 검증 (docs/DESIGN.md 참고)
2. **디자인 시스템 미참조**: UI 컴포넌트 설계 시 `../design-system`을 참조하지 않고 임의의 스타일/컴포넌트를 정의했는지 여부
3. **누락된 예외 케이스 (Edge Cases)**: 네트워크 장애, 잘못된 데이터 입력, 동시성 오류 등 발생 가능한 실패 시나리오 및 대응책 누락 여부
4. **보안/보안 정책 위반**: 인증/인가 누락, 비밀키 노출 위험 여부 (docs/SECURITY.md 참고)
...
| :--- | :--- | :--- | :--- |
| 예시: UI 구조 | `../design-system`을 참조하지 않고 독자적인 테마 컬러(Hex)를 직접 작성함 | 디자인 시스템의 컬러 토큰을 사용하도록 수정 요망 | High |
```
