...
if ($args.Count -eq 0) {
    Write-Error "Usage: init-harness.ps1 <project_name>"
    exit 1
}

$projectName = $args[0]

Write-Output "=== Starting Standardized Project Structure Setup for project: $projectName ==="
...
Create-DirIfNotExist $projectsDir
Create-DirIfNotExist $targetDir

# 3. Create folders
$directories = @(
    (Join-Path $targetDir ".claude"),
    (Join-Path $targetDir ".claude/skills"),
    (Join-Path $targetDir ".claude/agents"),
    (Join-Path $targetDir ".claude/guides"),
    (Join-Path $targetDir "docs"),
    (Join-Path $targetDir "docs/design-docs"),
    (Join-Path $targetDir "docs/exec-plans"),
    (Join-Path $targetDir "docs/exec-plans/active"),
    (Join-Path $targetDir "docs/exec-plans/completed"),
    (Join-Path $targetDir "docs/product-specs"),
    (Join-Path $targetDir "docs/references"),
    (Join-Path $targetDir "docs/generated"),
    (Join-Path $targetDir "tests")
)
...
'
Create-FileIfNotExist (Join-Path $targetDir "AGENTS.md") $agentsMd
...
'
Create-FileIfNotExist (Join-Path $targetDir "ARCHITECTURE.md") $architectureMd
...
'
Create-FileIfNotExist (Join-Path $targetDir ".claude/settings.json") $settingsJson
...
'
Create-FileIfNotExist (Join-Path $targetDir "package.json") $packageJson
...
```
'
Create-FileIfNotExist (Join-Path $targetDir ".claude/guides/pr-conventions.md") $prConventions
...
'
Create-FileIfNotExist (Join-Path $targetDir ".claude/guides/testing.md") $testingMd
...
'
Create-FileIfNotExist (Join-Path $targetDir "docs/design-docs/index.md") $designDocsIndex
...
'
Create-FileIfNotExist (Join-Path $targetDir "docs/design-docs/core-beliefs.md") $coreBeliefs
...
'
Create-FileIfNotExist (Join-Path $targetDir "docs/exec-plans/tech-debt-tracker.md") $techDebtTracker
...
'
Create-FileIfNotExist (Join-Path $targetDir "docs/DESIGN.md") $designMd
...
'
Create-FileIfNotExist (Join-Path $targetDir "docs/QUALITY_SCORE.md") $qualityScoreMd
...
'
Create-FileIfNotExist (Join-Path $targetDir "docs/RELIABILITY.md") $reliabilityMd
...
'
Create-FileIfNotExist (Join-Path $targetDir "docs/SECURITY.md") $securityMd

# Create .gitkeep files for empty folders
$gitkeepContent = "# Keep directory in git"
Create-FileIfNotExist (Join-Path $targetDir "docs/exec-plans/active/.gitkeep") $gitkeepContent
Create-FileIfNotExist (Join-Path $targetDir "docs/exec-plans/completed/.gitkeep") $gitkeepContent
Create-FileIfNotExist (Join-Path $targetDir "docs/product-specs/.gitkeep") $gitkeepContent
Create-FileIfNotExist (Join-Path $targetDir "docs/references/.gitkeep") $gitkeepContent
Create-FileIfNotExist (Join-Path $targetDir "docs/generated/.gitkeep") $gitkeepContent
Create-FileIfNotExist (Join-Path $targetDir "tests/.gitkeep") $gitkeepContent

Write-Output "=== Standardized Project Structure Setup Completed for: $projectName ==="
