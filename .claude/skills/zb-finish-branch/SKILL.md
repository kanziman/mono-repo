...
2-b. `[ ]` **시각 검증** (UI 컴포넌트를 포함한 프로젝트, 근거: [ui-plan-execution-visual-validation.md](../../projects/nsq/docs/solutions/workflow/ui-plan-execution-visual-validation.md)):
   - 변경된 파일 중 `.tsx`, `.jsx`, `page.tsx`, `layout.tsx`, `*.css` 등 UI 관련 파일이 있을 경우 이 단계를 수행합니다.
   - 에이전트가 직접 브라우저를 열 수 없을 경우 사용자에게 아래 메시지로 확인을 요청합니다:

     ```text
     [시각 검증 요청] 다음 페이지를 브라우저에서 확인해 주세요:
     - 변경된 페이지 목록 (예: /, /player, /import)
     - 확인 항목: 다크 배경 적용 여부, 카드/버튼 등 DS 컴포넌트 렌더링
     - 확인이 완료되면 계속 진행하겠습니다.
     ```

   - smoke-test.sh의 `[smoke] WARN: next-themes script not found` 경고가 출력됐을 경우, 이 단계를 건너뛰지 말고 반드시 사용자 확인을 받습니다.
